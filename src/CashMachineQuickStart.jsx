...
ss.textContent = `@keyframes spin { 0%{transform:rotate(0deg)} 100%{transform:rotate(360deg)} } @keyframes fadeUp { from{opacity:0;transform:translateY(12px)} to{opacity:1;transform:translateY(0)} }`;
    ...
    <div style={styles.modalOverlay} onClick={onClose}>
      <div style={styles.modalBox} onClick={e => e.stopPropagation()}>
        ...
        {phone.trim().length > 5 && !smsConsent && (
          <p style={{ fontSize: '11px', color: '#F06292', marginTop: '8px', textAlign: 'center' }}>Please check the SMS consent box above to continue with a phone number.</p>
        )}
...
// ── Week 1 Tasks Tab ──────────────────────────────────────────────────────────
const Week1Tab = ({ tasks }) => {
  if (!tasks) return <div style={styles.tabContent}><p style={{ color: 'rgba(255,255,255,0.5)' }}>Week 1 tasks are still generating...</p></div>;
  return (
    <div style={styles.tabContent}>
      <h3 style={styles.sectionTitle}>🚀 {tasks.headline}</h3>
      ...
      {tasks.tasks && tasks.tasks.map((t, i) => (
        <div key={i} style={{ display: 'flex', gap: '16px', padding: '16px 18px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', marginBottom: '12px' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: '0.8rem', fontFamily: '"IBM Plex Mono", monospace', color: '#3ECFAB' }}>⏱ {t.timeEstimate}</div>
          </div>
        </div>
      ))}
...
const CashMachineQuickStart = () => {
  ...
  const [activeTab, setActiveTab]           = useState('week1tasks');
...
No preamble.`);
  ...
  const generate90DayPlan = async () => {
    setError(''); setPhase(4);
    setPlan({ dualTrack: null, pricing: null, month1: null, month2: null, month3: null, marketing: null, milestones: null, week1Tasks: null });
    setActiveTab('week1tasks');
...
Person: ${name} | Good at: ${goodAt} | Time: ${timeAvailable} | Goal: ${incomeGoal}
These tasks will be emailed on Day 1 of the program. Make them concrete, doable, zero fluff.
Return ONLY valid JSON:
{"subject":"Week 1 Tasks: Your First 7 Days as a ${selectedIdea.title} Pro",
"headline":"Your Week 1 Starts Now, ${name}",
"intro":"1-2 sentences, warm coach voice, acknowledge they made a real decision today",
"tasks":[
  {"day":"Day 1–2","action":"specific task","why":"1 sentence reason","timeEstimate":"X hrs"},
  {"day":"Day 2–3","action":"specific task","why":"1 sentence reason","timeEstimate":"X hrs"},
  {"day":"Day 3–4","action":"specific task","why":"1 sentence reason","timeEstimate":"X hrs"},
        ...
        <div style={styles.phase}>
          {plan && plan.dualTrack && (
            <>
              ...
              {activeTab === 'week1tasks' && <Week1Tab tasks={plan.week1Tasks} />}
