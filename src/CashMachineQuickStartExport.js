// ─────────────────────────────────────────────────────────────────────────────
// CashMachineQuickStartExport.js
// Generates a clean, printable / PDF-saveable Cash Machine QuickStart plan
// No npm dependencies — uses browser print API
// ─────────────────────────────────────────────────────────────────────────────

export function exportCashMachineQuickStartPDF({ name, idea, plan, intake }) {
  ...
  // ── Parse plan sections into clean lines ──────────────────────────────────
  const renderLines = (text = "", accentColor = "#C9A84C") => {
...
<title>${name}'s Cash Machine Plan — ${idea.title}</title>
<style>
  /* ── Reset & Base ── */
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .bdot { color: #C9A84C; flex-shrink: 0; font-size: 8pt; margin-top: 2px; }
  ...
  .sh2 { font-size: 10.5pt; font-weight: 700; color: #0d1117; margin: 14px 0 5px; }
  .sh3 { font-size: 9pt; font-weight: 700; color: #C9A84C; text-transform: uppercase; letter-spacing: 0.08em; margin: 10px 0 4px; }
...
<div class="screen-nav no-print">
  <span class="screen-nav-title">📄 ${name}'s Cash Machine Plan — ${idea.title}</span>
...
<div class="page">
  <div class="brand-strip">
    <span class="brand-name">Cash Machine QuickStart · Loral Langemeier + Kelli Owens</span>
    <span class="brand-date">Generated ${date}</span>
  </div>

  <div class="cover-eyebrow">Your 90-Day Cash Machine Plan</div>
  <div class="cover-name">${name}</div>
  <div class="cover-sub">Built by the Cash Machine QuickStart · Loral Langemeier + Kelli Owens</div>

  <div class="idea-hero">
    <div class="idea-label">Your Cash Machine Idea</div>
  ...
  <div class="page-footer">
    <span class="footer-brand">Cash Machine QuickStart · Loral Langemeier + Kelli Owens</span>
...
<div class="page">
  <div class="brand-strip">
    <span class="brand-name">Cash Machine QuickStart · ${name}</span>
  ...
  ${renderLines(plan.pricing, "#C9A84C")}
...
<div class="page">
  ...
  <div style="margin-top:22px;background:#fdfbf5;border:1.5px solid #e8dfc5;border-radius:10px;padding:16px 18px;">
    <div style="font-family:'Courier New',monospace;font-size:7pt;letter-spacing:0.16em;text-transform:uppercase;color:#C9A84C;margin-bottom:6px;">The Only Rule</div>
    <div style="font-size:10pt;color:#333;line-height:1.7;font-style:italic;">"The only plan that doesn't work is the one you never start. Pick one idea. Say yes. Do it this week. Make money NOW, build for scale LATER."</div>
    <div style="font-family:'Courier New',monospace;font-size:7.5pt;color:#C9A84C;margin-top:8px;">— Loral Langemeier + Kelli Owens</div>
  </div>

  <div style="margin-top:18px;display:flex;justify-content:space-between;align-items:center;background:#0d1117;border-radius:8px;padding:12px 16px;">
    <div>
      <div style="font-family:'Courier New',monospace;font-size:7pt;letter-spacing:0.15em;text-transform:uppercase;color:#C9A84C;margin-bottom:3px;">Continue Your Journey</div>
      <div style="font-family:'Arial',sans-serif;font-size:9pt;color:#e8e4dc;">integratedwealthsystems.com · proactively-lazy.com</div>
    </div>
  </div>
